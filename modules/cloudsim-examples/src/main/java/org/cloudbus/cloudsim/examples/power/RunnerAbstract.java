...
public abstract class RunnerAbstract {
	...
	protected void initLogOutput(
			...
			String parameter,
			List<PowerHost> new_hostList,
			List<Vm> new_vmList) throws IOException, FileNotFoundException {
		setEnableOutput(enableOutput);
		hostList = new ArrayList<PowerHost>();
		hostList.addAll(new_hostList);
		vmList = new ArrayList<Vm>();
		vmList.addAll(new_vmList);
	...
	protected void start(String experimentName, String outputFolder, VmAllocationPolicy vmAllocationPolicy) {
		...
		try {
			...
			//List<Host> result = vmAllocationPolicy.getHostList();
			//Log.print("temperature is " + result.get(0).temperature);

			List<Cloudlet> newList = broker.getCloudletReceivedList();
			Log.printLine("Received " + newList.size() + " cloudlets");

			CloudSim.stopSimulation();
			Log.printLine("------------------" + "final temper" + "----------------------");
	...
	protected VmAllocationPolicy getVmAllocationPolicy(
		...
		if (vmAllocationPolicyName.equals("iqr")) {
			...
			vmAllocationPolicy = new PowerVmAllocationPolicySimple(hostList, vmAllocationPolicyName);
		} else if(vmAllocationPolicyName.equals("wf"))
		{
			vmAllocationPolicy = new PowerVmAllocationPolicyWF(hostList, vmAllocationPolicyName);
		}
		else if(vmAllocationPolicyName.equals("bf"))
		{
			vmAllocationPolicy = new PowerVmAllocationPolicyBF(hostList, vmAllocationPolicyName);
		}
		else if(vmAllocationPolicyName.equals("hybrid"))
		{
			vmAllocationPolicy = new PowerVmAllocationPolicyHybrid(hostList, vmAllocationPolicyName);
		}
		else {
